...
- (void)viewDidLoad 
{
    [super viewDidLoad];
     ...
     [[NSNotificationCenter defaultCenter] addObserver: self selector: @selector(cancel:) name: @"cancelMic" object: nil];
	...
	if (startStatus != EVoiceRecognitionStartWorking) // 创建失败则报告错误
	{
		...
		[self performSelector:@selector(firstStartError:) withObject:statusString afterDelay:3];  // 延迟0.3秒，以便能在出错时正常删除view
        return;
	}
...
#pragma mark 语音返回结果
               NSString *voiceNick=[appDelegate.appDefault objectForKey:@"voiceNick"]; //获得语音助手昵称
                 NSString *findWakeUp=[appDelegate.appDefault objectForKey:@"wakeUp"];//查看是否唤醒
                if ([findWakeUp isEqualToString:@"yes"]) {
                    //唤醒状态
                    
                        if (audioResultData.count>0) {
                            
                          for (int i=0; i<audioResultData.count; i++) {
                                NSString *str=audioResultData[i];
                              
                              [self.delegate showVoiceStr:str];
                                [DatabaseOperation queryVoiceKey:str hostId:[CurrentTableName shared].hostId voiceNumber:i];//数据库查询语音指令
                            }
                        }
                        
                    if([audioResultData containsObject:@"退下"]) {
                    
                    [self performSelector:@selector(exitVoice) withObject:nil afterDelay:3.0f];//退下
                    
                    
                    }else if ([[CurrentTableName shared].queryResult isEqualToString:@"no"]) {
                          
                            [CurrentTableName shared].noResultCount+=1;
                            
                            if ([CurrentTableName shared].noResultCount==4) {
                                //查询三次无结果（直接退下）
                            [self performSelector:@selector(exitVoice) withObject:nil afterDelay:3.0f];
                               
                                
                            }else{
                            //查询无结果
                       [self performSelector:@selector(playWhatToDo) withObject:nil afterDelay:1.0f];//播放：主人你还有什么吩咐
                            }
                        }else{
                            //查询有结果
                            [self performSelector:@selector(checkVoice) withObject:nil afterDelay:6.0f];//重新启动语音
                            
                        }
                    
                }else{
                 //没唤醒
                 if([audioResultData containsObject:voiceNick]) {
                    //呼叫是否包含语音助手昵称
                    [appDelegate.appDefault setObject:@"alwaysNo" forKey:@"micStatus"];//mic不用一直监听
                        
                [appDelegate.appDefault setObject:@"yes" forKey:@"queryVoice"];//设置“主人有人什么吩咐”标签
                [self performSelector:@selector(playMp) withObject:nil afterDelay:1.0f];//播放：您好！主人
                }else{
                       //呼叫语音助手失败
                        
                [self performSelector:@selector(checkVoice) withObject:nil afterDelay:3.0f];
...
#pragma  mark 超时,用户不说话
    NSString *micStatus=[appDelegate.appDefault objectForKey:@"micStatus"];//是否一直监听
                if ([micStatus isEqualToString:@"alwaysYes"]) {
                   
            [self performSelector:@selector(checkVoice) withObject:nil afterDelay:3.0f];//重新打开语音
                    
               }else{
                    
                NSString *queryVoice=[appDelegate.appDefault objectForKey:@"queryVoice"];//唤醒后是否说话
                    if ([queryVoice isEqualToString:@"no"]) {
                        //没说话，直接退下
              ...
              [self performSelector:@selector(exitVoice) withObject:nil afterDelay:3.0f];
                    ...
                    //没说话，直接吩咐
                         [CurrentTableName shared].noResultCount+=1;
                        [appDelegate.appDefault setObject:@"no" forKey:@"queryVoice"];
      
                [self performSelector:@selector(playWhatToDo) withObject:nil afterDelay:3.0f];
...
{	
	...
	switch (aStatus)
	{
        ...
        case EVoiceRecognitionClientNetWorkStatusEnd:  //网络工作完成
        {
          ...
          NSString *micStatus=[appDelegate.appDefault objectForKey:@"micStatus"];
...
-(void)playMp{
    ...
    // 获取文件路径
    NSString * soundFilePath = [[NSBundle mainBundle] pathForResource:@"master" ofType:@"mp3"];
    ...
    [appDelegate.appDefault setObject:@"yes" forKey:@"wakeUp"];
...
-(void)playWhatToDo{
    ...
    // 获取文件路径
    NSString * soundFilePath = [[NSBundle mainBundle] pathForResource:@"what_master" ofType:@"mp3"];
    NSURL * fileURL = [NSURL fileURLWithPath: soundFilePath];
    ...
    [self performSelector:@selector(checkVoice) withObject:nil afterDelay:6.0f];
...
-(void)exitVoice{
    ...
    // 获取文件路径
    NSString * soundFilePath = [[NSBundle mainBundle] pathForResource:@"im_calling" ofType:@"mp3"];
    NSURL * fileURL = [NSURL fileURLWithPath: soundFilePath];
    ...
    if  (!player2.playing){
        [player2 play];
        NSLog(@"播放成功！");
        NSLog(@"%f seconds played so  far",player2.currentTime);
  ...
  [self performSelector:@selector(checkVoice) withObject:nil afterDelay:6.0f];
     ...
     [appDelegate.appDefault setObject:@"alwaysYes" forKey:@"micStatus"];//开启一直监听
    [appDelegate.appDefault setObject:@"no" forKey:@"wakeUp"];//语音助手沉睡
